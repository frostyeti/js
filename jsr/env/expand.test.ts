...
test("expand::custom set function", () => {
    const captured: { key: string; value: string } = { key: "", value: "" };
    const { get, set } = createEnv();
    const options: SubstitutionOptions = {
        set: (key: string, value: string) => {
            captured.key = key;
...
test("expand::command substitution basic", () => {
    const { get, set } = createEnv();
    const result = expand('$(echo "hello")', get, set, { commandSubstitution: true });
    equal(result, "hello");
});
...
test("expand::command substitution with surrounding text", () => {
    const { get, set } = createEnv();
    const result = expand('The value is: $(echo "test")', get, set, { commandSubstitution: true });
...
test("expand::command substitution with variable expansion", () => {
    const { env, get, set } = createEnv();
    env["NAME"] = "world";
    const result = expand('$(echo "hello") ${NAME}', get, set, { commandSubstitution: true });
    equal(result, "hello world");
});

test("expand::command substitution multiple", () => {
    const { get, set } = createEnv();
    const result = expand('$(echo "a") and $(echo "b")', get, set, { commandSubstitution: true });
