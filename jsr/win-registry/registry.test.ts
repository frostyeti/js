...
test("win-registry::Registry create/open/set/get/delete (Windows)", { skip: !WINDOWS }, () => {
  // create key
...
test("win-registry::openKey relative operations (Windows)", { skip: !WINDOWS }, () => {
  const root = Registry.HKCU;
  const created = root.createKey("Software\\frostyeti-js-test-registry-relative");
  try {
    created.setString("Relative", "value");
    const child = root.openKey("Software\\frostyeti-js-test-registry-relative");
