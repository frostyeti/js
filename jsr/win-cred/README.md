...
Cross-runtime Windows Credential Manager module that works with Deno, Bun, and Node.js. Each runtime uses its own FFI mechanism to call the Win32 advapi32.dll credential functions directly -- no child processes or native addons required (except `koffi` for Node.js).
