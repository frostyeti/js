...
test(
    "chmod() sets read only permission bits on regular files",
    { skip: platform() === "win32" },
    async () => {
...
test(
    "fs::chmod() sets read only permission bits on a directory",
    { skip: platform() === "win32" },
    async () => {
        const tempDirPath = await mkdtemp(resolve(tmpdir(), "chmod_"));
...
test("fs::chmod() sets write only permission bits of regular file via symlink", {
    skip: platform() === "win32",
}, async () => {
    const tempDirPath = await mkdtemp(resolve(tmpdir(), "chmod_"));
...
test("fs::chmodSync() sets write only permission on a regular file via symlink", {
    skip: platform() === "win32",
}, () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "chmodSync_"));
    const testFile = resolve(tempDirPath, "chmod_file.txt");
