...
const RENAME_HAS_ISSUE = typeof globals.Deno !== "undefined" && globals.Deno.version &&
...
test("rename() succeeds when an existing directory is renamed with another directory path",
...
test("rename() rejects with Error when an existing directory is renamed with an existing regular file path",
...
test("rename() rejects with Error when renaming an existing directory with a valid symlink'd regular file path",
  { skip: platform() === "win32" },
  async () => {
    const tempDirPath = await mkdtemp(resolve(tmpdir(), "rename_"));
    const testDir = join(tempDirPath, "testDir");
...
test("rename() rejects with Error when renaming an existing directory with a valid symlink'd directory path",
  { skip: platform() === "win32" },
  async () => {
    const tempDirPath = await mkdtemp(resolve(tmpdir(), "rename_"));
    const testDir = join(tempDirPath, "testDir");
...
test("rename() rejects with Error when renaming an existing directory with a symlink'd file pointing to a non-existent file path",
  { skip: platform() === "win32" },
  async () => {
    const tempDirPath = await mkdtemp(resolve(tmpdir(), "rename_"));
    const testDir = join(tempDirPath, "testDir");
...
test("renameSync() throws with Error when an existing directory is renamed with an existing directory containing a file",
  { skip: RENAME_HAS_ISSUE },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
    const emptyDir = join(tempDirPath, "emptyDir");
...
test("renameSync() succeeds when an existing directory is renamed with another directory path",
  { skip: RENAME_HAS_ISSUE },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
    const testDir = join(tempDirPath, "testDir");
...
test("renameSync() throws with Error when an existing directory is renamed with an existing regular file path",
  // TODO(kt3k): This test should pass on Windows
  { skip: platform() === "win32" || RENAME_HAS_ISSUE },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
...
test("renameSync() throws with Error when renaming an existing directory with a valid symlink'd regular file path",
  { skip: platform() === "win32" },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
    const testDir = join(tempDirPath, "testDir");
...
test("renameSync() throws with Error when renaming an existing directory with a valid symlink'd directory path",
  { skip: platform() === "win32" },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
    const testDir = join(tempDirPath, "testDir");
...
test("renameSync() throws with Error when renaming an existing directory with a symlink'd file pointing to a non-existent file path",
  { skip: platform() === "win32" },
  () => {
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "renameSync_"));
    const testDir = join(tempDirPath, "testDir");
