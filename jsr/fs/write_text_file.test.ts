...
test("writeTextFile() can change the mode of a file",
  ...
  async () => {
    ...
    equal(getOctMode(fileStatBefore.mode! & 0o777), getOctMode(defaultFileMode), `Unexpected file mode: ${getOctMode(fileStatBefore.mode)} (${fileStatBefore.mode}) masked ${getOctMode(fileStatBefore.mode! & 0o777)} vs 0o644 ${0o644}`);
    ...
    equal(getOctMode(fileStatAfter.mode! & 0o777), getOctMode(0o222), `Unexpected file mode: ${getOctMode(fileStatAfter.mode)} (${fileStatAfter.mode}) masked ${getOctMode(fileStatAfter.mode! & 0o777)} vs 0o222 ${0o222}`);
...
function getOctMode(mode: number | undefined): string {
  if (mode === undefined) {
    return "undefined";
  }
...
test("writeTextFileSync() can change the mode of a file",
  ...
  () => {
    ...
    exists(fileStatBefore.mode, "mode is null");
    equal(fileStatBefore.mode! & 0o777, defaultFileMode, `Unexpected file mode: ${getOctMode(fileStatBefore.mode)} (${fileStatBefore.mode}) masked ${getOctMode(fileStatBefore.mode! & 0o777)} vs 0o644 ${0o644}`);

    writeTextFileSync(testFile, "Hello", { mode: 0o222 });
    const fileStatAfter = statSync(testFile);
    exists(fileStatAfter.mode, "mode is null");
    equal(fileStatAfter.mode! & 0o777, 0o222, `Unexpected file mode: ${getOctMode(fileStatAfter.mode)} (${fileStatAfter.mode}) masked ${getOctMode(fileStatAfter.mode! & 0o777)} vs 0o222 ${0o222}` );
