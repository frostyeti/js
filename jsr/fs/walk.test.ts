...
test("walk() walks unix socket", { skip: platform() === "win32" || !isDeno }, async () => {
    if (!isDeno) {
        return;
    }
...
// https://github.com/denoland/std/issues/1789
test("walkSync() walks unix socket", { skip: platform() === "win32" || !isDeno }, () => {
    if (!isDeno) {
        return;
    }
    const tempDirPath = mkdtempSync(resolve(tmpdir(), "deno_std_walk_sync_"));
...
test("walk() walks fifo files on unix", { skip: platform() === "win32" || !isDeno }, async () => {
    if (!isDeno) {
        return;
    }
...
test("walkSync() walks fifo files on unix", { skip: platform() === "win32" || !isDeno }, () => {
    if (!isDeno) {
        return;
    }
