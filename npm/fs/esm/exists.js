 ...
 * @example Basic usage
 * ```ts
 * import { exists } from "@frostyeti/fs";
 *
  ...
  try {
    ...
    if (globals.process && error instanceof Error) {
      ...
      if (code === "EPERM" || code === "EACCES") {
        if (
          WIN && code === "EACCES" && (options?.isDirectory || options?.isFile)
        ) {
    ...
    if (globals.Deno) {
      ...
      if (error instanceof globals.Deno.errors.PermissionDenied) {
        if (
          (await globals.Deno.permissions.query({ name: "read", path }))
            .state ===
            "granted"
        ) {
 ...
 * @example Basic usage
 * ```ts
 * import { existsSync } from "@frostyeti/fs";
 *
...
export function existsSync(path, options) {
  try {
    ...
    if (globals.Deno) {
      ...
      if (error instanceof globals.Deno.errors.PermissionDenied) {
        if (
          globals.Deno.permissions.querySync({ name: "read", path }).state ===
            "granted"
        ) {
          // --allow-read not missing
          return !options?.isReadable; // PermissionDenied was raised by file system, so the item exists, but can't be read
