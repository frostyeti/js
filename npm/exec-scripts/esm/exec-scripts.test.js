...
import { bash, BASH_SHELL_ARGS } from "./bash.js";
import { bun } from "./bun.js";
import { deno } from "./deno.js";
import { dotnet, DotnetScriptCommand } from "./dotnet.js";
import { go, GoScriptCommand } from "./go.js";
import { node } from "./node.js";
import { powershell, POWERSHELL_SHELL_ARGS } from "./powershell.js";
import { pwsh, PWSH_SHELL_ARGS } from "./pwsh.js";
import { python } from "./python.js";
import { ruby } from "./ruby.js";
import { sh, SH_SHELL_ARGS } from "./sh.js";
import { CmdScriptCommand } from "./wincmd.js";
const globals = globalThis;
const RUNTIME = globals.Deno ? "deno" : globals.Bun ? "bun" : "node";
function requireExe(t, name) {
...
test("exec-scripts::DotnetScriptCommand.getShellArgs selects mode by extension", (t) => {
  ...
  equal(cmd.getShellArgs("./proj.csproj", true), [
    "run",
    "--project",
    "./proj.csproj",
    "--",
...
test("exec-scripts::powershell.script runs and captures output", async (t) => {
  ...
  const output = await powershell.script("Write-Output 'hello from powershell'")
    .output();
  equal(output.code, 0);
...
test("exec-scripts::dotnet.script runs and captures output", async (t) => {
  ...
  equal(
    output.code,
    0,
    `Exit code should be 0. Output: ${output.text()} Error: ${output.errorText()}`,
  );
  if (WINDOWS) {
...
`);
  ...
  equal(
    output.code,
    1,
    `Exit code should be 1. Output: ${output.text()} Error: ${text}`,
  );
  ok(
    text.includes("exit status 7"),
    `Output should include "exit status 7". Output: ${text}`,
  );
});
