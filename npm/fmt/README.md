...
@frostyeti/fmt is cross runtime alternative to `@std/fmt` for sprintf and printf
functions. The `@std/fmt` module uses `Deno.inspect` and `Deno.stdout.writeSync`
for printf which is not available in bun, node, or the browser.

The `@frostyeti/fmt` module makes modifications to the `@std/fmt` module.

For inspect or the `/I` flag inspect uses `Deno.inspect`, `util.inspect` or
falls back to `json.parse`. **A browsify fallback may be included at a later
date**.

For writing to a stream for printf, print, etc; the @frostyeti/process module is
used which abstracts stdout for the different runtimes.
