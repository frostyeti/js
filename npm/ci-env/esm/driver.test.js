...
import { CI, CI_DRIVER, isInteractive, prependPath, setInteractive, setVar } from "./driver.js";
import {
  command,
  debug,
  endGroup,
...
test("ci-env::CI_DRIVER is a valid value", () => {
  ...
  ok(
    validDrivers.includes(CI_DRIVER),
    `CI_DRIVER "${CI_DRIVER}" is not a valid driver`,
  );
});
...
test("ci-env::setInteractive changes interactive state", () => {
  ...
  nope(
    isInteractive(),
    "Should be non-interactive after setInteractive(false)",
  );
  setInteractive(true);
