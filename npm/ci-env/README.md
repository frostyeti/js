...
The `ci-env` module enables determining if you are within a CI pipeline and
provides utilities for logging, setting environment variables, and handling
secrets across different CI tools (GitHub Actions, GitLab CI, Azure DevOps,
...
```typescript
import { prependPath, setVar } from "@frostyeti/ci-env";
...
```typescript
import { error, info, registerSecret } from "@frostyeti/ci-env";
...
```typescript
import { debug, info, isDebugEnabled, setDebug } from "@frostyeti/ci-env";
...
```typescript
import { info, isInteractive, setInteractive } from "@frostyeti/ci-env";
