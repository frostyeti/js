...
describe("assert::rejects", () => {
    ...
    test("passes when promise rejects with expected error type", async () => {
        await rejects(() => Promise.reject(new TypeError("test")), TypeError);
    });
    test("passes when promise rejects with matching message", async () => {
        await rejects(() => Promise.reject(new Error("test error message")), Error, "test error");
    });
    ...
    test("throws when error type doesn't match", async () => {
        let threw = false;
        try {
            await rejects(() => Promise.reject(new Error("test")), TypeError);
        }
        catch (e) {
            threw = true;
            equal(e instanceof AssertionError, true);
    ...
    test("throws with custom message", async () => {
        let message = "";
        try {
            await rejects(() => Promise.resolve(), "should have rejected");
        }
        catch (e) {
            message = e.message;
        }
    ...
    test("works with Error subclasses", async () => {
        ...
        await rejects(() => Promise.reject(new CustomError("custom")), CustomError);
    });
});
