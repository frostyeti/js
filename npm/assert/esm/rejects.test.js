...
describe("assert::rejects", () => {
    ...
    describe("error class matching", () => {
        test("passes when error matches expected class", async () => {
            await rejects(() => Promise.reject(new TypeError("type error")), TypeError);
        });
        ...
        test("throws when error class doesn't match", async () => {
            let threw = false;
            try {
                await rejects(() => Promise.reject(new TypeError("type error")), SyntaxError);
            }
            catch {
                threw = true;
            }
        ...
        test("passes with RangeError", async () => {
            await rejects(() => Promise.reject(new RangeError("out of range")), RangeError);
        });
        test("passes with ReferenceError", async () => {
            await rejects(() => Promise.reject(new ReferenceError("undefined variable")), ReferenceError);
        });
    });
    describe("message matching", () => {
        test("passes when message includes expected string", async () => {
            await rejects(() => Promise.reject(new Error("validation failed")), Error, "validation");
        });
        test("throws when message doesn't include expected string", async () => {
            let threw = false;
            try {
                await rejects(() => Promise.reject(new Error("validation failed")), Error, "not found");
            }
            catch {
                threw = true;
            }
        ...
        test("passes with partial message match", async () => {
            await rejects(() => Promise.reject(new Error("User not found in database")), Error, "not found");
        });
    });
    describe("custom message", () => {
        ...
        test("includes custom message in error", async () => {
            let message = "";
            try {
                await rejects(() => Promise.reject(new TypeError("type")), SyntaxError, undefined, "custom assertion message");
            }
            catch (e) {
                message = e.message;
            }
