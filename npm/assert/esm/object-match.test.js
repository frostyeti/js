...
import { objectMatch } from "./object-match.js";
...
describe("assert::objectMatch", () => {
    ...
    test("passes with nested objects", () => {
        objectMatch({ foo: { bar: 3, baz: 4 } }, { foo: { bar: 3 } });
        objectMatch({ a: { b: { c: 1, d: 2 } } }, { a: { b: { c: 1 } } });
    });
    test("passes with arrays inside objects", () => {
        objectMatch({ foo: [1, 2, 3], bar: true }, { foo: [1, 2, 3] });
        objectMatch({ items: [{ id: 1 }, { id: 2 }] }, { items: [{ id: 1 }] });
    });
    ...
    test("throws when nested values don't match", () => {
        let threw = false;
        try {
            objectMatch({ foo: { bar: 1 } }, { foo: { bar: 2 } });
        }
        catch (e) {
            threw = true;
            equal(e instanceof AssertionError, true);
    ...
    test("works with arrays of objects", () => {
        objectMatch({ users: [{ name: "Alice", age: 30 }, { name: "Bob", age: 25 }] }, { users: [{ name: "Alice" }] });
    });
});
