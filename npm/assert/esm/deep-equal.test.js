...
describe("assert::deepEqual", () => {
    ...
    describe("TypedArrays", () => {
        test("returns true for identical Uint8Arrays", () => {
            equal(deepEqual(new Uint8Array([1, 2, 3]), new Uint8Array([1, 2, 3])), true);
        });
        test("returns false for different Uint8Arrays", () => {
            equal(deepEqual(new Uint8Array([1, 2, 3]), new Uint8Array([1, 2, 4])), false);
        });
        test("returns false for different length TypedArrays", () => {
            equal(deepEqual(new Uint8Array([1, 2]), new Uint8Array([1, 2, 3])), false);
        });
        test("works with Int32Array", () => {
            equal(deepEqual(new Int32Array([1, -2, 3]), new Int32Array([1, -2, 3])), true);
        });
        test("works with Float64Array", () => {
            equal(deepEqual(new Float64Array([1.5, 2.5]), new Float64Array([1.5, 2.5])), true);
        });
        test("works with BigInt64Array", () => {
            equal(deepEqual(new BigInt64Array([1n, 2n, 3n]), new BigInt64Array([1n, 2n, 3n])), true);
        });
    });
    ...
    describe("URL objects", () => {
        test("returns true for identical URLs", () => {
            equal(deepEqual(new URL("https://example.com/path"), new URL("https://example.com/path")), true);
        });
        test("returns false for different URLs", () => {
            equal(deepEqual(new URL("https://example.com/a"), new URL("https://example.com/b")), false);
        });
    });
    ...
    describe("WeakMap and WeakSet", () => {
        test("throws TypeError for WeakMap", () => {
            throws(() => deepEqual(new WeakMap(), new WeakMap()), TypeError, "cannot compare WeakMap instances");
        });
        test("throws TypeError for WeakSet", () => {
            throws(() => deepEqual(new WeakSet(), new WeakSet()), TypeError, "cannot compare WeakSet instances");
        });
    });
    describe("WeakRef", () => {
