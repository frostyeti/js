...
import { decodeSecret, encodeSecret, WinCred } from "./mod.js";
import { WINDOWS } from "@frostyeti/globals/os";
import { CredPersist, CredType } from "./types.js";
...
test("win-cred::WinCred.read returns full credential object", {
  skip: !WINDOWS,
}, () => {
  try {
    WinCred.write({
...
test(
  "win-cred::WinCred.readSecret returns decoded string",
  { skip: !WINDOWS },
  () => {
    try {
      WinCred.write({
...
test("win-cred::WinCred.readSecret returns null for non-existent credential", {
  skip: !WINDOWS,
}, () => {
...
test(
  "win-cred::WinCred.delete removes a credential",
  { skip: !WINDOWS },
  () => {
    WinCred.write({
      targetName: TEST_TARGET,
...
test("win-cred::WinCred.delete returns false for non-existent credential", {
  skip: !WINDOWS,
}, () => {
  const deleted = WinCred.delete("non-existent-target-name-12345");
  equal(deleted, false);
});
test("win-cred::WinCred.enumerate returns matching credentials", {
  skip: !WINDOWS,
}, () => {
  try {
    WinCred.write({
...
test("win-cred::WinCred.enumerate with filter returns empty for non-matching", {
  skip: !WINDOWS,
}, () => {
...
test(
  "win-cred::WinCred.write with Uint8Array secret",
  { skip: !WINDOWS },
  () => {
    try {
...
test(
  "win-cred::WinCred.write updates existing credential",
  { skip: !WINDOWS },
  () => {
    try {
      WinCred.write({
